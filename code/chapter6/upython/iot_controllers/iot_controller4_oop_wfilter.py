# iot_controller4_oop.py
# dew.ninja  June 2023
# OOP version of iot_controller4.py

# OOP implementation (Sep 2023)
# controllers are implemented as classes on controllers.py
# NETPIE is implemented as class on netpie.py
...
import sys
from controllers import PIDControl, LLIControl, SFIControl
...
# ---- 4-point moving average filter using OOP -----
# ---------- works ok on LAG3 board but not on wokwi ---
mafilter = 1 # 0 = no filter, 1 = MA4 filter active

class MA:
    def __init__(self,numpts):
        self.numpts = numpts
        self.x = array.array('f',0.0 for _ in range (self.numpts))
        self.i = 0
        self.sumx = 0.0
        self.y = 0.0
    def compute(self,u):
        ...
        self.y = self.sumx /self.numpts; # compute average
        # update circular buffer index
        self.i+=1
        if self.i == self.numpts: 
            self.i = 0
        return self.y
...
# --------------------------------------------------------
online = True  # set to True to connect to NETPIE
if online:
...
# --------------------------------
# ------ OOP implementation of controller class------

                                                    
UMAX = 3.3  # limit max controller output to 3.3 vols
UMID = 1.65
UMIN = 0.0 # min controller output
T = 0.05  # sampling period
T_ms = int(1000*T)
t_data = 0  # time data output

# ----- create PID parameter instance -------
kp = 4.8  # PID gains
ki = 2.74
kd = 2.1
...
PID_controller = PIDControl(UMIN, UMAX, T,kp,ki,kd,kt,N,wp,wd)

# -- create lead-lag controller instance -------
ll_k = 250  # lead_lag controller gain 
ll_z = 1 # lead-lag zero
ll_p = 700 # lead-lag pole

LLI_controller = LLIControl(UMIN, UMAX, T, ll_k, ll_z, ll_p)

# ----- create state-feedback + integrator instance -----
K_sfic = [100.5, 152.25, 371.25]  # state feedback gain
#ff_gain = 324  # feedforward gain
Ki_sfic = 350 

SFI_controller = SFIControl(UMIN, UMAX, T, K_sfic, Ki_sfic)
...
def at_pid_adjust():
    ...
    PID_controller.setki(ki)
    PID_controller.setkp(kd)
...
def timer_isr(event):
    ...
    y_old = y
    if plantsim:
        yr,x2r,x1r = lag3(a,b,T,u, u_states, y_states) # simulation. in volt
    else:
        yr = y_in.read_uv()*1e-6 # from electronics
        x1r = x_1.read_uv()*1e-6
        x2r = x_2.read_uv()*1e-6

    if mafilter:
        y = y_ma.compute(yr)
        x1 = x1_ma.compute(x1r)
        x2 = x2_ma.compute(x2r)
    else:
        y = yr
        x1 = x1r
        x2 = x2r
    ...
    if feedback:
        if controller==0: # PID selected
            if autotune:
                ...
                ulim = PID_controller.out(r,y)
        elif controller==1: # output-feedback controller
            ulim = LLI_controller.out(r,y)
        elif controller==2: # state-feedback controller
            ulim = SFI_controller.out(r,x1,x2,y)
    else:
        if lsid: # LSID mode
            u = PRBS_generator()
            ulim = limit_and_offset(u)
...
def cmdInt(userstr):
    ...
    # addeed in ofc_pid_lag3.py June 2023
        ...
        if noparm==1:
            ...
            T_ms = int(1000*T)
            a,b = lag3_init(T)
            PID_controller.sett(T)
            PID_controller.update()
            LLI_controller.sett(T)
            LLI_controller.update()
            SFI_controller.sett(T)
        ...
        if noparm==1:
            ...
            if kt > 100: # set maximum kt
                ...
                kt = 0
            PID_controller.setkt(kt)
            PID_controller.update()
            update_dashboard()

    elif cmdstr.lower() == "wp":
        if noparm==1:
            print(PID_controller.getwp())
        else:            
            wp = float(parmstr)
            if wp > 10: # set maximum wp
                ...
                wp = 0
            PID_controller.setwp(wp)
            PID_controller.update()
            update_dashboard()

    elif cmdstr.lower() == "wd":
        if noparm==1:
            print(PID_controller.getwd())
        else:            
            wd = float(parmstr)
            if wd > 10: # set maximum wd
                ...
                wd = 0
            PID_controller.setwd(wd)
            PID_controller.update()
            update_dashboard()

    elif cmdstr.lower() == "n":
        if noparm==1:
            print(PID_controller.getN())
        else:            
            N = float(parmstr)
        ...
        print(u)
    elif cmdstr.lower() == "ulim":
        print(ulim)
    elif cmdstr.lower() == "ryu":
        print("r = "+str(r)+", y = "+str(y)+", u = "+str(u)+", ulim = "+str(ulim))
...
while True:
    if online:
        ...
        if publish_delta>PUBLISH_PERIOD: # publish interval must be larger than PUBLISH_PERIOD
            ...
            netpie.check_msg()
            # ----- add the following lined for OOP -------
            ...
            netpie.publish("@msg/shadowdata", csv_str)
            # added in iot_controller3.py
